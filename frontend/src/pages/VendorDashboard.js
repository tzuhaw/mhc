...
import { Logout as LogoutIcon, Visibility as VisibilityIcon } from '@mui/icons-material';
    ...
    <>
      <AppBar position="static">
        <Toolbar>
          <Typography variant="h6" component="div" sx={{ flexGrow: 1 }}>
            Vendor Dashboard - {user?.vendorName}
          </Typography>
      ...
      <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
          ...
          {getPendingCount() > 0 && (
            <Chip
              label={`${getPendingCount()} Pending Review`}
              color="warning"
              variant="outlined"
            />
          )}
        ...
        <Box mb={2}>
          <Typography variant="body2" color="textSecondary">
            Event Types: {user?.eventTypes?.join(', ')}
          </Typography>
        </Box>
                ...
                {events.length === 0 ? (
                  ...
                  events.map((event) => (
                    <TableRow 
                      ...
                      hover
                      sx={{
                        backgroundColor: event.status === 'Pending' ? 'rgba(255, 193, 7, 0.1)' : 'inherit'
                      }}
                    >
                      <TableCell>
